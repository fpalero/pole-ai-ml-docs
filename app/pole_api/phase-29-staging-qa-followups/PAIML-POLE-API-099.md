...
## Supplementary note (docs-unified keep-both): HEAD trimmed single-item variant

The unified-branch HEAD version of this ticket (44 lines) scoped 099 to item 1
only — "[Chatbot] Fix `OpenRouterLLM` test double: set `llm._timeout = 120.0` in
`test_openrouter_chat_sends_max_tokens`" (test-only fix, zero prod risk; root cause:
adopted deadline ticket `PAIML-POLE-API-095` made `OpenRouterLLM.chat()` read
`self._timeout`, breaking the `__new__`-built double in
`test_analyst_chatbot_093_followups.py` ~line 472). That content is fully covered
by item 1 above; the fuller 4-item bundle from
`feature/PAIML-POLE-API-099-reviewer-leftovers` is kept as canonical.
